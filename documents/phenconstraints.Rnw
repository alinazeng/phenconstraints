...
%JD: what about your randomisations? One way to introduce this into the Results is to discuss identities of species with unusual phenologies given their preceding event times and interphase durations etc.
...
\par Our findings have important implications for improved forecasting of climate change induced shifts in phenology. %JD: this reminds my of the winegrape work predicting harvest time, you have developed sequential models of budburst, flowering and veraison to predict harvest time - you cannot simply model later phenological events in isolation ...AKE:cite one or more ag models here.
First, a shift in one phase may have cascading effects on later phases, since each phase is linked to phases that occur before and after it \citep{wolkovich2014b}. Though phenology models covering the whole annual cycle have been developed for crop species (e.g., ),%AKE: Lizzie, do you have a favorite wine grape model that we could cite? 
